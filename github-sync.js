...
function readTask(value){
  ...
  if(company&&isUrl(url)){task={company,url};renderTask();if(accessToken)writeSource()}
}
function isUrl(value){try{return ["http:","https:"].includes(new URL(value).protocol)}catch{return false}}
function renderTask(){$("company").textContent=task.company||"等待公司信息";$("url").textContent=task.url||"--"}
...
async function writeSource(){
  if(!task.company||!isUrl(task.url)){status("公司名称或招聘网址无效。","error");return}
  ...
  try{
    ...
    doc.version=1;doc.updatedAt=new Date().toISOString();doc.sources=sources;
    await github(PATH,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:"Update "+task.company+" recruitment source",content:encode(doc),sha:current.sha})});
    status(task.company+" 已写入仓库，将从下一次每日任务开始监测。","ok");
    button.textContent="已写入仓库";
    channel.postMessage({type:"synced",company:task.company,url:task.url});
