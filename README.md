...
网站“重点公司与招聘源”区域会显示默认官方招聘网址，支持添加、修改和删除重点公司。浏览器修改可立即生效；首次使用“写入仓库”时，需要提供仅授权本仓库 Contents 读写权限的 fine-grained token。令牌只保存在独立同步窗口的内存中，不写入仓库或浏览器存储。

个人配置统一写入 `config/custom_sources.json`。新增或修改的网址会进入下一次每日监测；删除的公司写入 `removedCompanies`，前端和每日任务都会跳过。每日任务会使用大方向及细分领域关键词，从所有官方招聘源识别具体岗位链接；不能定位详情页时保留官方检索入口和岗位编号。
